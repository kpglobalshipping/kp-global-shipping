
import ExecutiveLeadership from "../components/ExecutiveLeadership";
import Image from "next/image";

export default function CompanyPage() {
  return (
    <main className="min-h-screen bg-slate-100">

      {/* =========================================================
          HERO
      ========================================================= */}

      <section className="bg-gradient-to-r from-blue-900 to-sky-700 text-white py-24">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-yellow-400 font-bold text-2xl">
            KP Global Shipping Inc.
          </p>

          <h1 className="text-6xl font-extrabold mt-4">
            Company Profile
          </h1>

          <p className="mt-8 text-xl text-blue-100 max-w-3xl">
            Quick, Prompt, and Stand-beside YOU.
          </p>
        </div>
      </section>


      {/* =========================================================
          ABOUT US
      ========================================================= */}

      <section className="py-24">
        <div className="max-w-7xl mx-auto px-6">

          <p className="text-yellow-500 font-bold text-3xl mb-3">
            About Us
          </p>

          <h2 className="text-5xl font-extrabold text-blue-900">
            Your Trusted Maritime Partner in the Philippines
          </h2>

          <p className="mt-8 text-xl leading-9 text-gray-700">
            KP Global Shipping Inc. is one of the Philippines&apos; leading
            maritime service providers offering Port Agency, Protective
            Agency, Crew Change, Husbandry Services, Technical Services,
            Marine Logistics, Customs Clearance, Ship Supply and Offshore
            Support throughout the country.
          </p>

          <p className="mt-6 text-xl leading-9 text-gray-700">
            Through our experienced operations team and nationwide network,
            we provide quick, prompt and reliable maritime services to
            shipowners, operators and charterers worldwide.
          </p>

        </div>
      </section>


      {/* =========================================================
          COMPANY AT A GLANCE
      ========================================================= */}

      <section className="bg-gray-100 py-24">
        <div className="max-w-7xl mx-auto px-6">

          <div className="text-center mb-16">

            <p className="text-yellow-500 font-bold text-3xl mb-3">
              Company at a Glance
            </p>

            <h2 className="text-5xl font-extrabold text-blue-900">
              Trusted Maritime Partner
            </h2>

            <p className="mt-6 text-xl text-gray-600 max-w-3xl mx-auto">
              Delivering dependable maritime solutions through experience,
              professionalism, and nationwide operational support.
            </p>

          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">

            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              <h3 className="text-5xl font-extrabold text-blue-900">
                2017
              </h3>

              <p className="mt-4 text-gray-600">
                Established
              </p>
            </div>

            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              <h3 className="text-5xl font-extrabold text-blue-900">
                40+
              </h3>

              <p className="mt-4 text-gray-600">
                Philippine Ports
              </p>
            </div>

            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              <h3 className="text-5xl font-extrabold text-blue-900">
                24/7
              </h3>

              <p className="mt-4 text-gray-600">
                Operations Support
              </p>
            </div>

            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              <h3 className="text-5xl font-extrabold text-blue-900">
                100%
              </h3>

              <p className="mt-4 text-gray-600">
                Customer Commitment
              </p>
            </div>

          </div>
        </div>
      </section>


      {/* =========================================================
          VISION / MISSION / CORE VALUES
      ========================================================= */}

      <section
        id="foundation"
        className="py-24 bg-[#D5E2EF]"
      >
        <div className="max-w-7xl mx-auto px-6">

          <div className="text-center mb-16">

            <p className="text-yellow-500 font-bold text-3xl mb-3">
              Our Foundation
            </p>

            <h2 className="text-5xl font-extrabold text-blue-900">
              Vision, Mission & Core Values
            </h2>

            <p className="mt-6 text-xl text-gray-600 max-w-3xl mx-auto">
              The principles that guide every service we deliver and every
              partnership we build.
            </p>

          </div>

          <div className="grid lg:grid-cols-3 gap-8">

            {/* Vision */}

            <div className="bg-[#123B5D] rounded-2xl p-10 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition duration-300">

              <h3 className="text-3xl font-bold text-white mb-6">
                Vision
              </h3>

              <p className="text-white leading-8">
                To become one of Asia&apos;s most trusted maritime service
                providers, recognized for operational excellence, innovation,
                integrity, and customer satisfaction.
              </p>

            </div>


            {/* Mission */}

            <div className="bg-[#123B5D] rounded-2xl p-10 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition duration-300">

              <h3 className="text-3xl font-bold text-white mb-6">
                Mission
              </h3>

              <p className="text-white leading-8">
                To provide reliable, quick, prompt, and professional maritime
                solutions through experienced people, modern technology,
                and nationwide operational support.
              </p>

            </div>


            {/* Core Values */}

            <div className="bg-[#123B5D] rounded-2xl p-10 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition duration-300">

              <h3 className="text-3xl font-bold text-white mb-6">
                Core Values
              </h3>

              <ul className="space-y-3 text-white">

                <li>✔ Integrity</li>
                <li>✔ Professionalism</li>
                <li>✔ Reliability</li>
                <li>✔ Teamwork</li>
                <li>✔ Customer Commitment</li>
                <li>✔ Continuous Improvement</li>

              </ul>

            </div>

          </div>
        </div>
      </section>


      {/* =========================================================
          CEO MESSAGE
      ========================================================= */}

      <section
        id="ceo-message"
        className="py-24 bg-blue-900 text-white"
      >
        <div className="max-w-7xl mx-auto px-6">

          <div className="grid lg:grid-cols-3 gap-16 items-center">

            {/* CEO PHOTO */}

            <div className="flex flex-col items-center justify-center">

              <div className="relative h-72 w-72 overflow-hidden rounded-full border-4 border-yellow-400 shadow-2xl">

                <Image
                  src="/images/charlie-park-2026.png"
                  alt="Charlie Park, Chief Executive Officer"
                  fill
                  priority
                  sizes="288px"
                  className="object-cover object-top"
                />

              </div>

              <h3 className="mt-6 text-2xl font-bold text-white">
                Charlie Park
              </h3>

              <p className="mt-1 text-sm font-semibold uppercase tracking-wider text-yellow-400">
                Chief Executive Officer
              </p>

            </div>


            {/* CEO MESSAGE */}

            <div className="lg:col-span-2">

              <p className="text-yellow-400 font-bold text-3xl mb-4">
                Message from the CEO
              </p>

              <h2 className="text-5xl font-extrabold mb-8">
                Leading with Trust,
                <br />
                Delivering with Excellence
              </h2>

              <p className="text-xl leading-7 text-blue-100">

                Welcome to KP Global Shipping.

                <br />
                <br />

                Since our establishment, our mission has been simple: to provide
                reliable, responsive, and professional maritime services that
                our clients can trust. In an industry where every hour counts
                and every decision matters, we believe that true service goes
                beyond completing a task—it is about standing beside our
                clients every step of the way.

                <br />
                <br />

                Our guiding principle, &quot;Quick, Prompt, and Stand-beside YOU,&quot;
                reflects who we are and how we serve. It is more than a slogan;
                it is our commitment to delivering timely solutions, maintaining
                open communication, and providing dependable support whenever
                and wherever our clients need us.

                <br />
                <br />

                The maritime industry continues to evolve, presenting new
                opportunities and challenges. At KP Global Shipping, we embrace
                innovation while remaining grounded in the values of integrity,
                accountability, professionalism, and teamwork. These principles
                enable us to build lasting partnerships with ship owners,
                operators, charterers, and business partners from around the
                world.

                <br />
                <br />

                Our greatest strength is our people. Their dedication, expertise,
                and unwavering commitment to excellence allow us to consistently
                deliver safe, efficient, and customer-focused services throughout
                the Philippines. Together, we strive not only to meet expectations
                but to exceed them.

                <br />
                <br />

                As we look toward the future, we remain committed to expanding
                our capabilities, strengthening our partnerships, and raising
                the standard of maritime agency and logistics services. Every
                vessel we serve and every client we support is an opportunity
                to demonstrate our commitment to excellence.

                <br />
                <br />

                On behalf of the entire KP Global Shipping family, thank you for
                your trust and confidence. We look forward to serving you with
                professionalism, integrity, and the unwavering commitment that
                defines who we are.

                <br />
                <br />

                Together, let us navigate the future with confidence.

              </p>

              <div className="mt-12">

                <h3 className="text-3xl font-bold">
                  Charlie Park
                </h3>

                <p className="text-yellow-300 text-xl mt-2">
                  Chief Executive Officer
                </p>

              </div>

            </div>

          </div>
        </div>
      </section>


      {/* =========================================================
          OUR GLOBAL NETWORK
      ========================================================= */}

      <section className="py-24 bg-gray-100">
        <div className="max-w-7xl mx-auto px-6">

          <div className="text-center mb-16">

            <p className="text-yellow-500 font-bold text-3xl mb-3">
              Our Global Network
            </p>

            <h2 className="text-5xl font-extrabold text-blue-900">
              KP Group
            </h2>

            <p className="mt-6 text-xl text-gray-600 max-w-3xl mx-auto">
              Connecting global principals with dependable maritime services
              throughout the Philippines through our experienced people and
              trusted operational network.
            </p>

          </div>


          {/* THREE NETWORK CARDS */}

          <div className="grid lg:grid-cols-3 gap-8">

            {/* Philippines */}

            <div className="bg-[#123B5D] rounded-2xl shadow-xl p-10 hover:shadow-2xl hover:-translate-y-2 transition duration-300 text-center">

              <div className="text-5xl font-extrabold text-white mb-6">
  KP
</div>

              <h3 className="text-3xl font-bold text-white">
                Philippines
              </h3>

              <p className="text-yellow-300 font-semibold mt-3">
                KP Global Shipping Inc.
              </p>

              <p className="mt-6 text-blue-100 leading-8">
                Headquarters providing Port Agency, Protective Agency,
                Crew Change, Husbandry Services, Technical Services,
                Marine Logistics and Customs Clearance.
              </p>

            </div>


            {/* Korea */}

            <div className="bg-[#123B5D] rounded-2xl shadow-xl p-10 hover:shadow-2xl hover:-translate-y-2 transition duration-300 text-center">

              <div className="text-5xl font-extrabold text-white mb-6">
  KP
</div>

              <h3 className="text-3xl font-bold text-white">
                Korea Branch
              </h3>

              <p className="text-yellow-300 font-semibold mt-3">
                JB Jang
              </p>

              <p className="mt-6 text-blue-100 leading-8">
                Supporting Korean principals, shipowners, operators,
                charterers and technical managers through close coordination
                with our Philippine operations team.
              </p>

            </div>


            {/* Global */}

            <div className="bg-[#123B5D] rounded-2xl shadow-xl p-10 hover:shadow-2xl hover:-translate-y-2 transition duration-300 text-center">

              <div className="text-6xl mb-6">
                🌍
              </div>

              <h3 className="text-3xl font-bold text-white">
                Global Network
              </h3>

              <p className="text-yellow-300 font-semibold mt-3">
                Worldwide Partners
              </p>

              <p className="mt-6 text-blue-100 leading-8">
                Serving international shipowners, operators, charterers,
                technical managers, and maritime organizations across the globe.
              </p>

            </div>

          </div>
        </div>
      </section>


      {/* =========================================================
          COMPANY TIMELINE
      ========================================================= */}

      <section
        id="timeline"
        className="py-24 bg-slate-100"
      >
        <div className="max-w-7xl mx-auto px-6">

          <div className="text-center mb-20">

            <p className="text-yellow-500 font-bold text-3xl mb-3">
              Our Journey
            </p>

            <h2 className="text-5xl font-extrabold text-blue-900">
              Company Timeline
            </h2>

            <p className="mt-6 text-xl text-gray-600 max-w-3xl mx-auto">
              Growing through dedication, trust, and continuous improvement
              in the maritime industry.
            </p>

          </div>


          <div className="relative max-w-5xl mx-auto">

            {/* Vertical Line */}

            <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-yellow-400 transform -translate-x-1/2"></div>


            {/* 2017 */}

            <div className="relative flex items-center mb-20">

              <div className="w-1/2 pr-10 text-right">

                <h3 className="text-4xl font-extrabold text-blue-900">
                  2017
                </h3>

                <p className="mt-3 text-gray-600">
                  KP Global Shipping Inc. was established in the Philippines.
                </p>

              </div>

              <div className="w-8 h-8 rounded-full bg-yellow-500 border-4 border-white shadow-lg z-10"></div>

              <div className="w-1/2"></div>

            </div>


            {/* 2020 */}

            <div className="relative flex items-center mb-20">

              <div className="w-1/2"></div>

              <div className="w-8 h-8 rounded-full bg-yellow-500 border-4 border-white shadow-lg z-10"></div>

              <div className="w-1/2 pl-10">

                <h3 className="text-4xl font-extrabold text-blue-900">
                  2020
                </h3>

                <p className="mt-3 text-gray-600">
                  Expanded nationwide operations and strengthened port agency
                  services.
                </p>

              </div>

            </div>


            {/* 2023 */}

            <div className="relative flex items-center mb-20">

              <div className="w-1/2 pr-10 text-right">

                <h3 className="text-4xl font-extrabold text-blue-900">
                  2023
                </h3>

                <p className="mt-3 text-gray-600">
                  Continued growth through international partnerships and
                  technical services.
                </p>

              </div>

              <div className="w-8 h-8 rounded-full bg-yellow-500 border-4 border-white shadow-lg z-10"></div>

              <div className="w-1/2"></div>

            </div>


            {/* 2026 */}

            <div className="relative flex items-center">

              <div className="w-1/2"></div>

              <div className="w-8 h-8 rounded-full bg-yellow-500 border-4 border-white shadow-lg z-10"></div>

              <div className="w-1/2 pl-10">

                <h3 className="text-4xl font-extrabold text-blue-900">
                  2026
                </h3>

                <p className="mt-3 text-gray-600">
                  Digital transformation through a modern corporate website
                  and enhanced client services.
                </p>

              </div>

            </div>

          </div>
        </div>
      </section>


      {/* =========================================================
          LEADERSHIP
      ========================================================= */}

      <div id="leadership">
        <ExecutiveLeadership />
      </div>

    </main>
  );
}

